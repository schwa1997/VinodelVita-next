...
import {
  getAllReportsAsAdmin,
  getAllVineyardsAsAgronomists,
} from "@/app/server/api/apis";
import { ReportType, VineyardType } from "@/app/type";
...
const ListReportsAsAgronomist = () => {
  ...
  const [vineyard, setVineyard] = useState<VineyardType>();
  console.log("reports", reports, "currentPage", currentPage);

  const handleVineYardsOption = (value: string) => {
    const filteredVineyards = vineyards?.filter(
      (vineyard) => vineyard.name === value
    );
  ...
  const handleAllReports = () => {
    setVineyard(undefined);
  };
  useEffect(() => {
    if (vineyard) {
      getAllReportsAsAdmin("createdAt", vineyard?.name, currentPage).then(
        (res) => {
